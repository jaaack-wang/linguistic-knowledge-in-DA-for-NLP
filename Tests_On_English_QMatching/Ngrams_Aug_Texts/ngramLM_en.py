...
class NgramLM:
    ...
    def __init__(self):
        self._unigram = readJson('../data/QuoraUnigram.json')
        self._bigram = readJson('../data/QuorBigram.json')
        self._trigram = readJson('../data/QuorFourgram.json')
        self._fourgram = readJson('../data/QuorTrigram.json')
    ...
    def sentProb(self, sent, trigram=False, bigram=False):
        assert len(sent) > 0, 'Input "sent" must not be empty!'
        if isinstance(sent, (list, tuple,)):
            tokens = sent
        elif isinstance(sent, str):
            tokens = BasicTokenizer().tokenize(sent)
        else:
    ...
    def rankSentsProbs(self, sentences, trigram=False, bigram=False, out_str=True):
        probs = [self.sentProb(tokens, trigram, bigram) for tokens in sentences]
    ...
    def pickBestSent(self, sentences, out_num=1, trigram=False, 
        ...
        if show_prob:
            return [res for res in self.rankSentsProbs(sentences, trigram, bigram, out_str)[:out_num]]
        return [sent for sent, _ in self.rankSentsProbs(sentences, trigram, bigram, out_str)[:out_num]]
